...
class ThreadingHazardsTests {
    /**
     * A simple (i.e. _plain_ [Int]) mutable field.
     */
    private var simpleCounterField = 0

    private fun incrementSimpleCounterMultipleTimes() {
        repeat(N_OF_REPS) {
            simpleCounterField += 1
        }
    }
    ...
    fun `loosing increments - using separate method`() {
        ...
        val threads = List(N_OF_THREADS) {
            threadBuilder.start(this::incrementSimpleCounterMultipleTimes)
        }
        // and: waiting for these threads to end
        threads.forEach(Thread::join)

        // then: the final counter value is NOT N_OF_THREADS * N_OF_REPS
        assertNotEquals(N_OF_THREADS * N_OF_REPS, simpleCounterField)
    }

    /**************************************************************************
     * Similar to the previous test, but using a captured local variable
     * instead of a field.
     */
    @Test
    fun `loosing increments - using a lambda and captured context`() {
        // when: creating N_OF_THREADS that each increment simpleCounter N_OF_REPS times
        var simpleCounterLocal = 0
        val threads = List(N_OF_THREADS) {
            threadBuilder.start {
                // note that this code runs in a different thread
                repeat(N_OF_REPS) {
                    simpleCounterLocal += 1
                }
            }
        }
        ...
        // then: the final counter value is NOT N_OF_THREADS * N_OF_REPS
        assertNotEquals(N_OF_THREADS * N_OF_REPS, simpleCounterLocal)
    }

    /**
     *J ust a simple stack using a linked list.
     */
