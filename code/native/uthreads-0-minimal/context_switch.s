 ...
 *  - %rdi has curr_thread, i.e., the pointer to the current thread descriptor.
 *  - %rsi has next_thread, i.e., the pointer to the next thread descriptor.
 */
context_switch:
    // suspend curr_thread ...
    ...
    // resume next_thread ...
    // ... by loading the next_thread stack from the next_thread descriptor
	movq (%rsi), %rsp

	// from here on, rsp points to the next_thread stack.
...
context_switch_and_free:
    ...
    // Note: the call to free is done on the next thread stack
