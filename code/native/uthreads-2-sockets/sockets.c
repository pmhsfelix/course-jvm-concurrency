...
void move_running_thread_to_list(list_entry_t *list);
...
socket_t *server_socket_accept(server_socket_t *ss)
{
  while (1)
  {
    ...
    {
      // ready -> not-ready, since there is no client socket ready to accept
      move_running_thread_to_list(&(ss->accept_threads));
      schedule();
    }
    else
    {
  ...
  while (1)
  {
    ...
    else
    {
      if (errno == EAGAIN || errno == EWOULDBLOCK)
      {
        ...
        move_running_thread_to_list(&(socket->writer_threads));
        move_running_thread_to_list(&(socket->writer_threads));
        schedule();
      }
      else
  ...
  while (1)
  {
    ...
    else
    {
      if (errno == EAGAIN || errno == EWOULDBLOCK)
      {
        ...
        move_running_thread_to_list(&(socket->reader_threads));
        schedule();
      }
      else
      {
